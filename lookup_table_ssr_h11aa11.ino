// Pin Definitions
const int TRIAC_PIN = 9;          // Pin to control TRIAC (via MOC3021)
const int ZERO_CROSS_PIN = 2;      // Zero-cross detection from H11AA1
const int POT_PIN = A0;            // Potentiometer for speed control

// Comprehensive lookup table for 0° to 180° (delay times in microseconds)
...
void loop() {
  ...
  // potentiometer value to index of the lookup table (0-180 for 181 values)
  int index = map(potValue, 0, 1023, 0, 180);

  //The corresponding delay time from the lookup table
  int delayTime = lookupTable[index];
...
void zeroCrossISR() {
  // Wait for the delay time based on the lookup table
  int potValue = analogRead(POT_PIN);
  int index = map(potValue, 0, 1023, 0, 180);
  int delayTime = lookupTable[index];
  ...
  // Fire the TRIAC via the MOC3021
